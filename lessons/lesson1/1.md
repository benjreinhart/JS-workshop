...
In JavaScript, an array actually behaves just like an object:

```
var array_literal = ['zero', 'one', 'two'];
...
array_literal[0]; // 'zero'
array_object[0]; // 'zero'

array_literal[3] = 'three';
array_object[3] = 'three';
...
// That I know of, in most other languages, the index used for accessing must be an integer

array_literal['3']; // 'three'

```

What happens is that the JavaScript interpreter actually converts integer types to a string and stores them as a property with an associated value, just like an the JavaScript `Object`. Since JavaScript objects aren't super performant in terms of lookup times, arrays in JavaScript do not have the lookup time performance benefits they do in many other languages.

Although arrays in JavaScript are treated more like JavaScript objects internally, they inherit properties from `Array`'s prototype (`Array.prototype`), and not `Object`'s prototype (`Object.prototype`) which gives them a handful of useful methods that allow us to treat them like arrays in other languages. They also have their own literal format. We'll talk more about this when we talk about prototypes, methods and inheritance.
