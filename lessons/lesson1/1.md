...
A new instance of an object can be created by using the `new` keyword with a function that acts as a constructor. This means we can treat `Shape` like a class, and it is strict convention to capitalize a function that'll be used as a constructor. By adding a function onto `Shape`'s prototype, we are creating a *method*. Now anytime we create a new instance of Shape (by using the `new` keyword), our instance has direct access to that method. Also, when using `this` inside these methods, `this` refers to the instance that you are calling the method on.
