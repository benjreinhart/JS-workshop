...
These are really nothing new if you've worked with any other language.
...
Then there is the `for..in` loop. This loop is designed for iterating over objects. Because arrays are objects in JavaScript, it works for iteration over them too, however, it is significantly less performant than a regular `for` loop. So if you're iterating over arrays, use the `for` loop, if you're iterating over other objects, use the `for..in`.
...
`obj.prototype` is undefined in the example above because `obj` does not have it's own prototype, but it contains a hidden link to its parent's prototype through `__proto__`. We don't ever use `__proto__` explicitly.
...
// _NOT_ on null or undefined though:
...
Prototype chains are how we link objects together, and methods and properties defined on a `prototype` will be available to any object that shares the same `prototype`.
