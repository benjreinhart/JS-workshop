...
The `String` object wraps the primitive data type `string`, providing it with a number of methods and properties. The `Number` object does the same for the primitive data type `number`. `Object` and `Array` objects provide methods and properties for JavaScript's built-in data structures. `Object` also also the granddaddy of any JavaScript object, and all other objects inherit from it.
...
In JavaScript, the primitive data type `numeric` is actually a little different than most programming language's version of a numerical primitive. In JavaScript, `numberic` is the only numberical type. There is no distinction between a floating point decimal and an integer. This means that `1` and `1.0` are the same. Many other languages (everyone that I've ever used) has a distinction here, which means if you want to perform arithmetic involving a decimal point, you'd have to explicitly specify that it is a float or use some conversion methods. This is avoided in JavaScript and actually saves a lot of headaches.
