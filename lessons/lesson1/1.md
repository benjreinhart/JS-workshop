...
Object literals in JavaScript are key-value pairs. They can be accessed by either square bracket notation `[]` or by the dot `.` notation. However, the dot notation has a few caveats. The key and value pair is separated by a colon `:`.
...
Objects can contain any combination of date types and values, and key-value pairs are separated by commas.
