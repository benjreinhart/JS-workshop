...
The `String` object wraps the primitive data type `string`, providing it with a number of methods and properties. The `Number` object does the same for the `numeric` primitive data type. `Object` and `Array` objects provide methods and properties for JavaScript's built-in data structures. `Object` also also the granddaddy of any JavaScript object, and all other objects inherit from it.
