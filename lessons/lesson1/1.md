...
```
function Shape(type) {
  // creating a shape with the 'new' keyword, 'this' will refer to 
  // the newly created object (in other words, the new instance of the Shape class)
  this.type = type;
}

// By attaching a function to Shape's prototype, we are creating a method
// that can be used on an instance of the shape class.
Shape.prototype.getType = function() {
  // if called on an instance of the Shape class, 'this' will refer to the instance
  return this.type;
}

// now we create an instance of the Shape class, called square.
var square = new Shape("square");

square.getType(); // square
```

A new instance of an object can be created by using the `new` keyword with a function that acts as a constructor. This means we can treat `Shape` like a class, and it is strict convention to capitalize a function that'll be used as a constructor. By adding a function on to `Shape`'s prototype, we are creating a *method*. Now anytime we create a new instance of Shape (by using the `new` keyword), our instance has direct access to that method. Also, when using `this` inside these methods, `this` refers to the instance that you are calling the method on.

But what if we want to have `Shape` as a parent class and use inheritance to have sub classes, like squares, circles and rectangles?

```
// constructors
var Square = function(lengthOfSides) {
...
// Make the Circle and Square classes inherit from Shape by
// setting their prototype to a new instance of Shape
Square.prototype = new Shape("square");
Circle.prototype = new Shape("circle");

Square.prototype.area = function() {
  return this.lengthOfSides * this.lengthOfSides;
}
...
var square = new Square(10); // create a new instance of the Square class
square.getType(); // square
square.area(); // 100
...
This was nothing but an incredibly brief overview of JavaScript. It was only intended to be a quick overview though, so I recommend reading a number of the resources I outlined on the homepage for each of the previous sections to gain a much more thorough understanding of the language.
