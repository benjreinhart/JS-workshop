...
Did you notice something weird about `array.length`? It's missing the parenthesis `()` for normal method invocation. That's because `length` is just a property on a JavaScript object and not a method. That's right, arrays in JavaScript are nothing more than JavaScript objects (with the exception that they have their own literal format).
